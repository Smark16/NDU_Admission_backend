...
def save_draft_applications(request):
    try:
        ...
        draft.program_choices.all().delete()

        for index, p in enumerate(programs_input, start=1):
            try:
                pid = int(p)

                if pid > 0:
                    DraftProgramChoice.objects.create(
                        draft=draft,
                        program_id=pid,
                        choice_order=index
                    )
...
def get_draft_application(request):
    try:
        ...
        data = {
            ...
            "programs": list(draft.program_choices.order_by("choice_order").values_list("program_id", flat=True)),

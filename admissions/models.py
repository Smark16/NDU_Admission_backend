...
class IDCard(models.Model):
    ...
    verification_token = models.CharField(max_length=80, unique=True, db_index=True)
    ...
    def __str__(self):
        return f"{self.card_number} - {self.admitted_student.student_id}"
...
class IDCardPrintLog(models.Model):
    ...
    id_card = models.ForeignKey(IDCard, on_delete=models.CASCADE, related_name="logs")
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["id_card", "action"]),
            models.Index(fields=["performed_at"]),
        ]

    def __str__(self):
        return f"{self.id_card.card_number} - {self.action}"

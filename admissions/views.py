...
class ListAdmittedStudents(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # "continuing" includes imports (they are continuing students by nature).
        ...
        if student_origin and student_origin != "all":
            ...
            if student_origin in ("legacy", "legacy_imported", "imported"):
                ...
                queryset = queryset.filter(admitted_batch__is_active=True).exclude(
                    legacy_src_q
                )
            elif student_origin == "continuing":
                queryset = queryset.filter(
                    Q(admitted_batch__is_active=False) | legacy_src_q
                )
...
class AdminDashboardStats(APIView):
    ...
    def get(self, request):
        ...
        # Admitted students split so bulk-imported (legacy) rows can never
        # inflate the live-intake figure, even when mistagged onto an active
        # admission intake:
        #   - current intake: active batch, NOT a legacy import (genuine new admits)
        #   - continuing: everyone else — prior intakes AND bulk-imported
        #     continuing students (imports are continuing students by nature)
        #   - legacy imported: the "of which imported" subset of continuing
        ...
        return Response({
            ...
            # All continuing students: prior intakes + bulk-imported.
            "admittedContinuing": admitted_intake_stats['admitted_continuing'],
            # Of which bulk-imported from the old system (subset of continuing).
...
class AdminExemptionLineReopenView(APIView):
    """
    HOD / Dean / AR: undo one paper's decision without clearing the whole stage.

    POST /api/admissions/change_requests/<pk>/exemption_lines/<line_id>/reopen
    Body: { "stage": "hod"|"dean"|"ar", "reason"?: str }
    """
    ...
    def post(self, request, pk, line_id):
        from admissions.exemption_services import reopen_exemption_line_review
        from admissions.exemption_stages import user_can_review_exemption_stage
        from admissions.serializers import AdmissionChangeRequestSerializer
        ...
        if not user_can_review_exemption_stage(request.user, stage):
            return Response(
                {"detail": f"You do not have permission to reopen papers at the {stage.upper()} stage."},
                status=403,
            )
        scoped = filter_admission_change_requests_for_user(
            AdmissionChangeRequest.objects.filter(pk=req_obj.pk),
        ...
        try:
            with transaction.atomic():
                result = reopen_exemption_line_review(
                    req_obj,
                    stage=stage,
                    line_id=line_id,
                    actor=request.user,
